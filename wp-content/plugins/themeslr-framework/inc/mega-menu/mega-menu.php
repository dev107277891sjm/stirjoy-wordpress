...
class CF_Mega_Menus {
	...
	static function add_hooks() {
		...
		add_filter('nav_menu_css_class', 'CF_Mega_Menus::echo_nav_li_class_columns', 1, 3);
	...
	static function display_mega_menu_contents($output, $item, $depth, $args) {
		...
		if (empty($args['hide_mega_menu']) && $depth == 0 && empty($item['has_children'])) {
			...
			if (!empty($mega_menu_id) && ($mega_menu = get_post($mega_menu_id)) && !is_wp_error($mega_menu)) {
				...
				$mega_menu_columns_val = '';
				if ($mega_menu_columns) {
					$mega_menu_columns_val = '';
				}
	...
	static function echo_nav_li_class_columns($classes, $item, $args) {
	    $mega_menu_columns = get_post_meta($item->ID, '_cf_mega_menu_columns', true);
	    $mega_menu_columns_val = '';
	    if ($mega_menu_columns) {
	        $mega_menu_columns_val = $mega_menu_columns;
	    }
	    $classes[] = esc_attr($mega_menu_columns);
	    return $classes;
	}
	
	static function edit_nav_menu_walker($walker_classname) {
		include_once(CF_MEGA_MENUS_DIR.'lib/class.mega-menu.php');
	...
	static function save_mega_menu_setting($menu_id, $menu_item_id) {
		...
		// Columns
		$mega_menu_columns = false;
		if (!empty($_REQUEST['menu-item-mega-menu-columns']) && !empty($_REQUEST['menu-item-mega-menu-columns'][$menu_item_id])) {
			$mega_menu_columns = $_REQUEST['menu-item-mega-menu-columns'][$menu_item_id];
		}
		if (!empty($mega_menu_columns)) {
			update_post_meta($menu_item_id, '_cf_mega_menu_columns', $mega_menu_columns);
		}
		else {
			delete_post_meta($menu_item_id, '_cf_mega_menu_columns');
		}
	}
