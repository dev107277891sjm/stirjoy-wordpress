		...
		if ( ! $is_pro ) {
			...
			$render_products_list = function ( $product_ids, $is_pro_local ) {
				...
				foreach ( $product_ids as $pid ) {
					...
					$short_description = $_p->get_short_description();
				    if ( empty( $short_description ) ) return;
				    $limited_description = wp_trim_words( $short_description, 5, '...' );

				    $prep_time = get_post_meta( $_p->get_id(), '_prep_time', true );
				    $product_url = $_p->get_permalink();

				    $tagHtml = '';
			        $product_tags = get_the_terms( $_p->get_id(), 'product_tag' );
					...
					echo '<div class="wps_sfw_sub_box_prod_tag">' . $tagHtml . '</div>';
					echo '<div class="wps_sfw_sub_box_prod_image">' . wp_kses_post( $img ) . '</div>';
					echo '<div class="wps_sfw_sub_box_prod_info">';
					echo '<div class="wps_sfw_sub_box_prod_name">' . esc_html( $name ) . '</div>';
					echo '<div class="wps_sfw_sub_box_prod_desc">' . esc_html( $limited_description ) . '</div>';
					echo '<div class="wps_sfw_sub_box_prod_meta">' . 
							'<div class="prep-time">' . 
								'<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-clock w-3 h-3 mr-1 text-muted-foreground" aria-hidden="true"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg>' .
								'<span>' . esc_html( $prep_time ) . ' min</span>' . 
							'</div>' .
							'<div class="meal-size">' . 
								'<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-users w-3 h-3 mr-1 text-muted-foreground" aria-hidden="true"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"></path><path d="M16 3.128a4 4 0 0 1 0 7.744"></path><path d="M22 21v-2a4 4 0 0 0-3-3.87"></path><circle cx="9" cy="7" r="4"></circle></svg>' .
								'<span>2</span>' . 
							'</div>' .
							'<div class="review">' . 
								'<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-star w-3 h-3 mr-1 text-yellow-400 fill-current" aria-hidden="true"><path d="M11.525 2.295a.53.53 0 0 1 .95 0l2.31 4.679a2.123 2.123 0 0 0 1.595 1.16l5.166.756a.53.53 0 0 1 .294.904l-3.736 3.638a2.123 2.123 0 0 0-.611 1.878l.882 5.14a.53.53 0 0 1-.771.56l-4.618-2.428a2.122 2.122 0 0 0-1.973 0L6.396 21.01a.53.53 0 0 1-.77-.56l.881-5.139a2.122 2.122 0 0 0-.611-1.879L2.16 9.795a.53.53 0 0 1 .294-.906l5.165-.755a2.122 2.122 0 0 0 1.597-1.16z"></path></svg>' .
								'<span>4.8</span>' . 
							'</div>' .
						 '</div>';
					echo '<div class="wps_sfw_sub_box_prod_price">' . $_p->get_price_html() . '</div>';
					echo '<div class="wps_sfw_sub_box_prod_qty">';
					echo '<a href="' . esc_html( $product_url ) . '" class="wps_sfw_sub_box_prod_view_details" target="_blank">View Details</a>';
					echo '<button class="wps_sfw_sub_box_prod_minus_btn" style="display:none;" data-product-id="' . esc_attr( $pid ) . '"><span>-</span> &nbsp;Remove</button>';
					if ( $is_pro_local ) {
					...
					echo '<button class="wps_sfw_sub_box_prod_add_btn" data-product-id="' . esc_attr( $pid ) . '"><span>+</span> &nbsp;Add</button>';
					echo '</div>';
					echo '</div>';
					echo '</div>';
				}
