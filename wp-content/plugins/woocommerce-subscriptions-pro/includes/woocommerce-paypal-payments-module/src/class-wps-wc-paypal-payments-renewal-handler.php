<?php
/**
 * Handles WPS Subscription renewals.
 *
...
class WPS_WC_PayPal_Payments_Renewal_Handler {
	...
	public function pay_renew( \WC_Order $renewal_order ): bool {
		...
		if ( ! $subscription_id || 'yes' !== $is_a_renew ) {
            $renewal_order->add_order_note( __( 'Sorry, no subscription was found or order is not a renew.', 'woocommerce-subscriptions-pro' ) );
			return false;
		}
		...
		try {
			...
			if ( 'false' !== $renewal_order->get_meta( AuthorizedPaymentsProcessor::CAPTURED_META_KEY ) ) {
				...
				throw new Exception( sprintf( __( 'Payment for order #%s already captured', 'woocommerce-subscriptions-pro' ), $order_id ) );
			}

			if ( ! $this->authorized_payments_processor->capture_authorized_payment( $renewal_order ) ) {
				// translators: %s stand for the order ID.
				throw new Exception( sprintf( __( 'Capture payment for order #%s failed', 'woocommerce-subscriptions-pro' ), $order_id ) );
			}
	...
	private function get_order( \WC_Order $wc_order ) {
		...
		if ( $token ) {
			if ( $wc_order->get_payment_method() === CreditCardGateway::ID ) {
				...
				return $this->order_endpoint->create(array($purchase_unit), $shipping_preference, $payer, '', array(), $payment_source);
			}

			if ( $wc_order->get_payment_method() === PayPalGateway::ID ) {

				return $this->order_endpoint->create(array($purchase_unit), $shipping_preference, $payer, '', array(), $token->to_payment_source());
			}
		}
