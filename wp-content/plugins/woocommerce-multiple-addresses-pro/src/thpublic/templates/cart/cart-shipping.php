...
$formatted_destination    = isset($formatted_destination) ? $formatted_destination : THWMA_Utils::thwma_get_formatted_address($package['destination'], ', ');
...
if(!empty($cart_data) && is_array($cart_data)){
	foreach ($cart_data as $cart_item) {
		...
		if ((!$_product-> is_virtual('yes')) && (!$downloadable)) {
		...
		if(isset($cart_item['product_shipping_address']) && !empty($cart_item['product_shipping_address'])) {
			...
			// $multi_address_change = urldecode(THWMA_Public::get_posted_value('multi_shipping_adr_data'));
			...
			$pdt_shipp_addr_formated = THWMA_Utils::set_ship_to_section_address($adr_data);		
			$shipp_addr_format = THWMA_Utils::get_formated_address('shipping',$adr_data);
			$values = THWMA_Utils::get_address_field_values($adr_data, $_product);
		} else {
			$default_address = '';
			if(is_user_logged_in()) {
				...
				$pdt_shipp_addr_formated = THWMA_Utils::set_ship_to_section_address($default_address);
				$values = THWMA_Utils::get_address_field_values($default_address, $_product);
			}
		}
		...
		WC()->shipping->calculate_shipping(THWMA_Utils::get_shipping_packages($shipping_values));
		$shipping_methods = WC()->shipping->packages;
		$ship_method = WC()->shipping->shipping_methods;

		$ship_method_free_ship = [];
		$ship_method_flat_rate = [];
		$set_amount = [];
		$item_total = isset($cart_item['line_total']) ? $cart_item['line_total'] : '';
		
		foreach ($ship_method as $key => $value) {
			if(isset($value->min_amount) && 'free_shipping' != $key){
				$ship_method_free_ship[$key] = $value->min_amount;
			}
		...
		// check a minimun order amount (free shipping).
		if(!empty($item_total) && !empty($ship_method_free_ship) && $item_total <= min($ship_method_free_ship)){
			if(!empty($shipping_methods) && is_array($shipping_methods)) {
				$method_id = min($ship_method_free_ship);
				$index_id = array_search($method_id, $ship_method_free_ship);
				unset($shipping_methods[$index]['rates']['free_shipping:'.$index_id]);
			}
		}

		// Flat rate not calculation while setting cost as Product amount*qty.
		if (!empty($set_amount) && isset($shipping_methods[0]['rates'])) {
			foreach ($shipping_methods[0]['rates'] as $key => $value) {
				if ('flat_rate' == $value->method_id && isset($value->id)) {
					$new_amount = min($set_amount) * $cart_quantity;
					$shipping_methods[0]['rates'][$value->id]->set_cost($new_amount);
				}
			}
		}
		    ...
		    //foreach($shipping_methods as $key => $methods) {
		    	if(isset($shipping_methods[$index]['rates'])) {
					...
					<tr class="woocommerce-shipping-totals shipping thwma-woo-shipping-totals">
						<th><?php echo wp_kses_post('Ship to','woocommerce-multiple-addresses-pro'); ?></th>
